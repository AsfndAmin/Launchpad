...
import "./DecentrapadFairLaunchWithoutAffiliate.sol"; 
...
contract DecentraFairLaunchFactory is AccessControl, ReentrancyGuard{
    ...
    function deployPool(
        DecentrapadFairLaunchWithoutAffiliate.launchSaleInfo memory _launchInfo,
        DecentrapadFairLaunchWithoutAffiliate.dexListingInfo memory _listingInfo,
        address owner,
        bool feesInNative, 
        ...
        DecentrapadFairLaunchWithoutAffiliate.poolFee memory _poolFee;

        if (feesInNative) {
            _poolFee = DecentrapadFairLaunchWithoutAffiliate.poolFee(nativeFee, 0);
            totalTokensRequired = calculateTotalForNative(_launchInfo.tokensForSale, nativeFee, _listingInfo.lpTokensRatio);
        } else {
            _poolFee = DecentrapadFairLaunchWithoutAffiliate.poolFee(dualFees, dualFees);
            totalTokensRequired = calculateTotalForDual(_launchInfo.tokensForSale, dualFees, _listingInfo.lpTokensRatio);
        } 

        DecentrapadFairLaunchWithoutAffiliate decentraPool = new DecentrapadFairLaunchWithoutAffiliate(
            _launchInfo,
            _listingInfo,
            _poolFee, 
            owner,
    ...
    function calculateTotalForNative(uint256 _tokensForSale, uint256 _fee, uint256 _lpTokenRatio) private view returns(uint256){
    ...
    function calculateTotalForDual(uint256 _tokensForSale, uint256 _dualFee, uint256 _lpTokenRatio) private view returns(uint256){
        uint256 tokensForLP = _tokensForSale.mul(_lpTokenRatio).div(base);
